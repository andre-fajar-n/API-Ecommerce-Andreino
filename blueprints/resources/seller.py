...
class Seller(Resource):
    ...
    def post(self):
        ...
        seller = SellerModel(args['nama'], args['email'],args['alamat'], args['no_hp'], claims['id'])
        db.session.add(seller)
        db.session.commit()

        app.logger.debug('DEBUG : %s', seller)

        return marshal(seller, SellerModel.response_fields), 200, {'Content-Type': 'application/json'}
    ...
    def patch(self):
        ...
        return marshal(qry, SellerModel.response_fields), 200, {'Content-Type': 'application/json'}
